...
def test_resolve_config() -> None:
    loaded_config: dict[str, str | Path] = {
        ...
        'cur_insert_sql_path': 'db/sql/cur/batch_cur_shipment_products.sql'
    }
    ...
    assert isinstance(result.cur_insert_sql_path, Path)
    ...
    assert result.cur_insert_sql_path == _REPO_ROOT_PATH.joinpath(loaded_config['cur_insert_sql_path'])
    # table checks
